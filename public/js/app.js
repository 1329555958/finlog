...
define('app', ['util', 'base'], function (util) {
    ...
    var app = window.APP = angular.module('app', ['ngExtend', 'ui.router', 'ui.ace', 'ngSanitize']);

    app.getCurrentTimeslot = function () {
        return angular.store(STORE.CONDITION);
    };
    /**
     * 获取当前条件的时间范围
     * @returns {startTime:ms,endTime:ms}
     */
    app.getCurrentTimeRange = function (_timeslot) {
        var timeRange = null;
        if (!TIMESLOT_IS_CUSTOM) {//app.js
            var startTime = _.calcTime(_timeslot);
            var endTime = new Date();
            timeRange = {startTime: startTime.getTime(), endTime: endTime.getTime()}
        } else {
            timeRange = _.chain(_timeslot).pick('startTime', 'endTime').each(function (val, key, t) {
                t[key] = dateUtil.parse(val).getTime();
            }).value();
        }
        console.debug('timerange', timeRange);
        return timeRange;
    }
    /**
     * 弹出对话框
